...
def add(pts, label, goodness):
    score_details.append((label, ("+" + str(pts)) if pts >= 0 else str(pts),
                           "#22c55e" if goodness == "green" else "#f97316" if goodness == "orange" else "#ef4444"))
    return pts

score += add(18 if rule_of_40 > 40 else 6,
             "Rule of 40: " + f"{rule_of_40:.1f}" + "%",
             "green" if rule_of_40 > 40 else "orange")
score += add(12 if fcf_yield > 3 else 4,
             "FCF Yield: " + f"{fcf_yield:.1f}" + "%",
             "green" if fcf_yield > 3 else "orange")
score += add(10 if gross_margin > 55 else 5,
             "Bruttomarge: " + f"{gross_margin:.1f}" + "%",
             "green" if gross_margin > 55 else "orange")
if rule_of_40 > 50:
    score += add(5, "Rule of 40 Bonus (> 50%)", "green")
if pe_to_use > 65:
    score += add(-16, "Forward P/E zu hoch (" + f"{pe_to_use:.0f}" + "x)", "red")
elif pe_to_use > 45:
    score += add(-9, "Forward P/E erhoeht (" + f"{pe_to_use:.0f}" + "x)", "orange")
if debt_to_equity > 2.0:
    score += add(-8, "Hohe Verschuldung (D/E " + f"{debt_to_equity:.2f}" + "x)", "red")
if beta > 1.6:
    score += add(-7, "Hohe Volatilitaet (Beta " + f"{beta:.2f}" + ")", "red")

score = max(0, min(score, 45))
status    = ("ELITE-QUALITAET" if score >= 36 else "Gute Qualitaet"
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "Ueberblick", "Growth Chart", "Finanzentwicklung", "Bilanz", "Bewertung & Risiko"
])
...
with tab1:
    ...
    st.markdown(
        ...
        "<h2 style=\"margin:0;color:" + hex_color + "\">" + str(score) + "/45</h2>"
    ...
    st.divider()
    st.markdown("**Score-Aufschluesselung**")
    for lbl, pts, clr in score_details:
        st.markdown("<span style=\"color:" + clr + ";font-weight:600\">" + pts + "</span>&nbsp; " + lbl,
                    unsafe_allow_html=True)
...
with tab2:
    ...
    st.caption("Unter EMA 200 = Kaufzone  |  Ueber EMA 200 = Zu teuer")
    c1,c2,c3 = st.columns(3)
    with c1:
...
with tab4:
    ...
    with c2: st.metric("Cash & Aequivalente", fmt_b(total_cash))
    ...
    with c1: st.metric("Debt/Equity",  f"{debt_to_equity:.2f}" + "x" if debt_to_equity else "N/A",
                        delta="OK" if debt_to_equity<1 else "Moderat" if debt_to_equity<2 else "Hoch")
    with c2: st.metric("Current Ratio", f"{current_ratio:.2f}" + "x" if current_ratio else "N/A",
                        delta="Liquide" if current_ratio>1.5 else "Knapp")
...
with tab5:
    ...
    with c3: st.metric("PEG Ratio",    f"{peg_ratio:.2f}" + "x"   if peg_ratio>0   else "N/A",
                        delta="Guenstig" if 0<peg_ratio<1.5 else "Teuer" if peg_ratio>=1.5 else "")
    c1,c2,c3 = st.columns(3)
    ...
    ampel(0<pe_to_use<=30, pe_to_use<=50,
          ...
          "Erhoehte Bewertung (P/E " + f"{pe_to_use:.1f}" + "x)",
          "Sehr teuer (P/E " + f"{pe_to_use:.1f}" + "x)" if pe_to_use>0 else "P/E nicht verfuegbar")
    ampel(debt_to_equity<1, debt_to_equity<2,
          "Geringe Verschuldung (D/E " + f"{debt_to_equity:.2f}" + "x)",
          "Moderate Verschuldung (D/E " + f"{debt_to_equity:.2f}" + "x)",
          "Hohe Verschuldung (D/E " + f"{debt_to_equity:.2f}" + "x)" if debt_to_equity>0 else "D/E nicht verfuegbar")
    ampel(gross_margin>55, gross_margin>30,
          "Starke Bruttomarge (" + f"{gross_margin:.1f}" + "%)",
          "Mittlere Bruttomarge (" + f"{gross_margin:.1f}" + "%)",
          "Niedrige Bruttomarge (" + f"{gross_margin:.1f}" + "%)")
